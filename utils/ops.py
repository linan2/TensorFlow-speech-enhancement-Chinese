...
def gaussian_noise_layer(input_layer, std):
    ...
    if shape[1] is None:
        shape[1] = 1
    noise = tf.random_normal(#shape=[shape[-1]],
                             shape=shape,
                             mean=0.0,
                             stddev=std,
                             dtype=tf.float32)
...
def exponential_decay(iteration, num_jobs, num_ietrs, init_learning_rate,
                      multiply_jobs=True):
    final_learning_rate = 0.0001 * init_learning_rate
    if iteration + 1 >= num_ietrs:
        current_learning_rate = final_learning_rate
    else:
        current_learning_rate = (
            init_learning_rate * math.exp(
                iteration * math.log(final_learning_rate / init_learning_rate)
                    / num_ietrs))
    if multiply_jobs:
        return num_jobs * current_learning_rate
    else:
        return current_learning_rate

def softmax_cross_entropy_with_logits(real_logits, fake_logits):
    """Broadcast version."""
    real_labels = tf.concat(
        [tf.ones_like(real_logits[:,:,0:1]) * 1.0,
         tf.ones_like(real_logits[:,:,0:1]) * 0.0], -1)
    fake_labels = tf.concat(
        [tf.ones_like(fake_logits[:,:,1:2]) * 0.0,
         tf.ones_like(fake_logits[:,:,1:2]) * 1.0], -1)
    class_real_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
        labels=real_labels, logits=real_logits))
    class_fake_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
        labels=fake_labels, logits=fake_logits))
    return class_real_loss + class_fake_loss
