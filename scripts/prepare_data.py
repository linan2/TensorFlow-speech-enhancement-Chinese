...
def calculate_mixture_features(args):
    ...
    reverb_speech_dir = args.reverb_speech_dir
    ...
    # reverb_vadio_path = os.path.join(workspace, "mixture_csvs", "%s.csv" % data_type)
    reverb_vadio_path = "workspace/mixture_csvs/train.csv"
    with open(reverb_vadio_path, 'rb') as f:
        reader = csv.reader(f, delimiter='\t')
        reverb_lis = list(reader)
		
    #clean_vadio_path = os.path.join(workspace, "mixture", "%s.csv" % data_type)
	clean_vadio_path = "workspace/mixture_csvs/train.csv"
    with open(clean_vadio_path, 'rb') as f:
        clean_reader = csv.reader(f, delimiter='\t')
        clean_lis = list(clean_reader)
    t1 = time.time()
    cnt = 0
    for i1 in xrange(1, 7861):
        [reverb_speech_na] = reverb_lis[i1]
        [clean_speech_na] = clean_lis[i1]
        # noise_onset = int(noise_onset)
        # noise_offset = int(noise_offset)
        
        # Read speech audio. 
        reverb_speech_path = os.path.join(reverb_speech_dir, reverb_speech_na)
        clean_speech_path = os.path.join(clean_speech_dir, clean_speech_na)
        (reverb_speech_audio, _) = read_audio(reverb_speech_path, target_fs=fs)
        (clean_speech_audio, _) = read_audio(clean_speech_path, target_fs=fs)
        # Read noise audio. 
        ...
        out_bare_na = os.path.join("%s" % 
             (os.path.splitext(reverb_speech_na)[0]))
        # out_audio_path = os.path.join(workspace, "mixed_audios", "spectrogram", 
            # data_type, "%s.wav" % out_bare_na)
        ...
        # Write out features. 
        out_feat_path = os.path.join("workspace", "features", "spectrogram", data_type,"%s.p" % out_bare_na)
        create_folder(os.path.dirname(out_feat_path))
        print out_feat_path
        data = [mixed_complx_x, speech_x]
        #print data
...
def calculate_test_features(args):
    workspace = args.workspace
    ...
    #mixture_csv_path = os.path.join(workspace, "mixture_csvs", "%s.csv" % data_type)
    # reverb_vadio_path = os.path.join(workspace, "mixture_csvs", "%s.csv" % data_type)
    for root, dirs, files in os.walk(speech_dir, topdown=False):
        for filename in fnmatch.filter(files, "*.wav"):
            wav_file = os.path.join(root, filename)
            (reverb_speech_audio, _) = read_audio(wav_file, target_fs=fs)
			         
				
        # # reverb_lis = list(reader)
    # for i1 in xrange(1, 186):
        # [reverb_speech_na] = reverb_lis[i1]
        # #[clean_speech_na] = clean_lis[i1]
    
        # # Read speech audio. 
        # reverb_speech_path = os.path.join(speech_dir, reverb_speech_na)
            #(reverb_speech_audio, _) = read_audio(reverb_speech_path, target_fs=fs)
        ...
        # Extract spectrogram. 
            mixed_complx_x = calc_sp(reverb_speech_audio, mode='complex')
            speech_x = calc_sp(reverb_speech_audio, mode='magnitude')
...
def pack_features(args):
    ...
    feat_dir = os.path.join("workspace", "features", "spectrogram", data_type)
    names = os.listdir(feat_dir)
    for na in names:
        # Load feature. 
        feat_path = os.path.join(feat_dir, na)
        data = cPickle.load(open(feat_path, 'rb'))
        [mixed_complx_x, speech_x] = data
    ...
    out_path = os.path.join("workspace", "packed_features", "spectrogram", data_type, "data.h5")
...
def compute_scaler(args):
    ...
    # Load data. 
    t1 = time.time()
    hdf5_path = os.path.join("workspace", "packed_features", "spectrogram", data_type, "data.h5")
    ...
    # Write out scaler. 
    out_path = os.path.join("workspace", "packed_features", "spectrogram", data_type, "scaler.p")
    create_folder(os.path.dirname(out_path))
...
if __name__ == '__main__':
    ...
    # parser_calculate_mixture_features = subparsers.add_parser('calculate_test_features')
    parser_calculate_mixture_features.add_argument('--workspace', type=str, required=True)
    parser_calculate_mixture_features.add_argument('--clean_speech_dir', type=str, required=True)
    #parser_calculate_mixture_features.add_argument('--speech_dir', type=str, required=True)
    parser_calculate_mixture_features.add_argument('--reverb_speech_dir', type=str, required=True)
