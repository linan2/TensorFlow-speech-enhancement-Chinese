...
def _griffin_lim(S):
	...
	y = signal.spectral.istft(S,nperseg=512,noverlap=256)
	for i in range(100):
                theta = np.angle(signal.spectral.stft(y,nperseg=512,noverlap=256))
		tmp = S_complex * np.exp(1j * theta)
		y = signal.spectral.istft(tmp)
	return y
	#angles = np.exp(2j * np.pi * np.random.rand(*S.shape)
...
def _stft(y):
	...
	#else:
	return librosa.stft(y=y, hop_length=256, win_length=512)

def _istft(y):
	return librosa.istft(y, hop_length=256, win_length=512)
