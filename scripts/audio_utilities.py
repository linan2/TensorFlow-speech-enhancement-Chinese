...
def stft_for_reconstruction(x, fft_size, hopsamp):
    ...
    hopsamp = int(hopsamp)
#    [f,t,x2] = scipy.signal.spectral.spectrogram(x,window=window,nperseg=512,noverlap=256,detrend=False,return_onesided=True,mode='complex')
#    return x2
...
def reconstruct_signal_griffin_lim(magnitude_spectrogram, cln_speech,fft_size, hopsamp, iterations):
    ...
    x_fft = cln_speech
    reconstruction_angle1 = np.angle(x_fft)
    proposal_spectrogram1 = magnitude_spectrogram*np.exp(1.0j*reconstruction_angle1)
    x_reconstruct = istft_for_reconstruction(proposal_spectrogram1, fft_size, hopsamp)
